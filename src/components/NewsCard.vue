...
const classDiv = computed(() => {
    return store.styleType === 'byFlex' ? 'max-w-[320px]' : '';
});

const classImg = computed(() => {
    return store.styleType === 'byFlex' ? 'w-[320px] h-[190px]' : '';
});

</script>

<template>
    <div :class="[classDiv, 'my-5 p-[10px] bg-white shadow-xl rounded-xl']">
        <a :href="article.url" target="_blank">
            <div>
                <img :src="article.urlToImage" alt="article img" :class="classImg" class="object-cover rounded-xl">
            </div>
            <div>
                <h2 class="font-medium py-4 px-[6px]">{{ store.styleType === 'byFlex' ? shortenedTitle : article.title
                    }}
                </h2>
                <p class="text-[14px] text-[#3E3232BF] px-[6px] pb-4">{{ store.styleType === 'byFlex' ?
                    shortenedDescription ||
                        shortenedDescription === '[Removed]' ? shortenedDescription :
                        "The former partner of Ugandan athlete Rebecca Cheptegei, who is accused of killi..." :
            ...
            <div class="flex gap-[10px]">
                ...
                <div class="text-[#3E3232BF] flex flex-col justify-between">
                    <p class="font-medium text-[14px]">
                        {{ store.styleType === 'byFlex' ? shortenedAuthor ? shortenedAuthor : 'Random Name' :
                            article.author }}
                    </p>
                    <p class="text-[12px]">
                        {{ formattedDate }}
